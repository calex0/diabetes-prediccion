...
# =========================
# 1. Variables de RFE (diabetes sí/no)
# =========================
...
# =========================
# 2. Combobox con categorías BRFSS
# =========================
...
clf = pipeline.named_steps["model"]   # ✔ tu pipeline usa "model", no "clf"
...
def construir_vector_entrada(valores_dict):
    # Crear un diccionario con todas las columnas del modelo
    data = {}
    ...
    # Devolver un DataFrame con nombres de columnas
...
def agrupar_shap_por_variable_original(shap_values_full):
    ...
    for var in FEATURE_NAMES:
        cols_var = [i for i, name in enumerate(columnas_transformadas)
                    if var in name]
        if not cols_var:
            shap_por_var[var] = 0.0
        else:
            shap_por_var[var] = shap_values_full[cols_var].sum()
    return np.array([shap_por_var[var] for var in FEATURE_NAMES])
...
class CribadoDiabetesApp(tk.Tk):
    ...
    def _crear_formulario(self):
        ...
        for i, name in enumerate(FEATURE_NAMES):
            ttk.Label(frame, text=name).grid(row=i, column=0, sticky="w", padx=5, pady=5)

            if name in opciones_combobox:
                valores_texto = list(opciones_combobox[name].values())
        ...
        try:
            for name, widget in self.inputs.items():
                if isinstance(widget, ttk.Combobox):
                    texto = widget.get()
                    for codigo, descripcion in opciones_combobox[name].items():
                        if descripcion == texto:
                            valores[name] = float(codigo)
                            break
                else:
                    val_str = widget.get().strip()
                    if val_str == "":
                        raise ValueError(f"El campo {name} está vacío.")
                    valores[name] = float(val_str)
