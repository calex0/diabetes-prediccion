...
mapa_en = {
    "_RFHLTH": "General physical/mental health",
    "_AGE_G": "Age group",
    "_BMI5CAT": "BMI category",
    "_RACEG21": "Racial/ethnic group",
    "WTCHSALT": "Salt intake control",
    "ASBIRDUC": "Alcohol consumption reduction",
    "CTELNUM1": "Number of telephones",
    "CTELENUM": "Telephone availability",
    "SCNTMEL1": "Days of poor mental health"
}

# Cargar columnas reales del modelo
columnas_rl = joblib.load("load/columnas_rl.pkl")

# Crear FEATURE_NAMES en español respetando el orden real
FEATURE_NAMES = [mapa_en[col] for col in columnas_rl]
...
pipeline = joblib.load("load/modelo_rl.pkl")
Xb_trans = joblib.load("load/X_background.pkl")
...
def mostrar_grafico_shap(shap_values, feature_names):
    ...
    plt.xlabel("SHAP value (contribution to diabetes risk)")
    plt.title("SHAP explanation of the prediction")
    plt.tight_layout()
    plt.show()
...
def generar_texto_explicativo(prob, clase, shap_values, feature_names):
    ...
    texto = []
    texto.append(f"Estimated probability of type 2 diabetes: {prob:.2f}")
    texto.append(f"Risk category: {riesgo_cat}")
    texto.append(f"Model classification: {'Diabetes (1)' if clase == 1 else 'No diabetes (0)'}")

    texto.append("\nFactors that most influence your risk:")
    ...
    for i in indices[:4]:
        ...
        valor = shap_values[i]
        if valor > 0:
            texto.append(f" - {nombre}: increases the risk (SHAP = {valor:.3f})")
        else:
            texto.append(f" - {nombre}: decreases the risk  (SHAP = {valor:.3f})")

    texto.append("\nThis result is a screening, not a diagnosis.")
    return "\n".join(texto)
...
class CribadoDiabetesApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Type 2 diabetes screening tool (RL + SHAP)")
    ...
    def _crear_formulario(self):
        frame = ttk.LabelFrame(self, text="Screening data (RFE variables)")
        frame.pack(fill="x", padx=10, pady=10)

        for i, col in enumerate(columnas_rl):
            nombre_es = mapa_en[col]
    ...
    def _crear_botones(self):
        ...
        ttk.Button(frame, text="Calculate risk", command=self._on_predict).pack(side="left", padx=5)
        ttk.Button(frame, text="View SHAP chart", command=self._on_show_shap).pack(side="left", padx=5)

    def _crear_area_resultados(self):
        frame = ttk.LabelFrame(self, text="Outcome and explanation")
        ...
        try:
            for col, widget in self.inputs.items():
                if isinstance(widget, ttk.Combobox):
                    ...
                    if val_str == "":
                        raise ValueError(f"The field {mapa_es[col]} is blank.")
    ...
    def _on_show_shap(self):
        if not hasattr(self, "_last_shap_values"):
            messagebox.showinfo("Info", "Please calculate a prediction first.")
            return
